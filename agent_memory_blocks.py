...
This script fetches all agents and blocks from the Letta API and creates
...
def get_api_key_from_settings() -> str:
    ...
    try:
        ...
        settings_path = os.path.expanduser("~/.letta/settings.json")
...
def get_letta_client() -> Letta:
    ...
    if not api_key:
        print("LETTA_API_KEY not found in environment, checking ~/.letta/settings.json...")
...
def fetch_agents_and_blocks(
    ...
    if use_cache:
        ...
        try:
            ...
            print(f"  Loaded relationships for {len(agent_to_blocks)} agents")
    ...
    if not use_cache:
        ...
        print("Building agent-block relationships...")
        agent_to_blocks = {}
        for agent in agents:
            try:
                ...
                print(f"  Warning: Failed to fetch blocks for agent {agent.id}: {e}")
...
def create_visualization(
    ...
    # Add agent nodes
    print(f"  Adding {len(agents)} agent nodes...")
    for agent in agents:
    ...
    # Add block nodes
    print(f"  Adding {len(blocks)} block nodes...")
...
def cleanup_orphaned_blocks(
    ...
    if client is None:
        print("\n❌ Error: API client required for cleanup. Do not use --cached without valid API key.")
        return
...
def main():
    ...
    try:
        ...
        if not args.cached or args.cleanup:
            print("Initializing Letta API client...")
        ...
        if lib_dir.exists() and lib_dir.is_dir():
            ...
            print(f"  Moved lib directory to {cache_lib_dir}")
        ...
        if args.cleanup:
            cleanup_orphaned_blocks(client, agents, blocks, agent_to_blocks)
